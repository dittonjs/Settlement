...
export default class UserInfoPanels extends React.Component{
  constructor(){
    super();
    this.state = {};
  ...
  getInvList(tab, inventory, styles){
    ...
    return _.map(data, (item)=>{
      return <div key={tab + item.name}style={styles.label} onClick={()=>{this.displayItem(item)}}>{item.name}</div>
    });
  }
  ...
  getMyBases(bases, styles){
    var bases = _.map(bases.baseData, (data, index)=>{
                return <div key={"bases_"+index}style={styles.label}>{data.name}</div>
              });
